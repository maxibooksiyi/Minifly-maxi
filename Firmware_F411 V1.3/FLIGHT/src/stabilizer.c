...
void stabilizerTask(void* param)
{
	...
	while(1)      //maxi:果然，一个任务里面就是一个while(1)
	{
		vTaskDelayUntil(&lastWakeTime, MAIN_LOOP_DT);		/*1ms周期延时*/

		//获取6轴和气压数据（500Hz）
		if (RATE_DO_EXECUTE(RATE_500_HZ, tick))   //maxi:#define RATE_DO_EXECUTE(RATE_HZ, TICK) ((TICK % (MAIN_LOOP_RATE / RATE_HZ)) == 0)  //maxi:就是把MAIN_LOOP_RATE用1000替代掉。
