...
typedef struct
{
	...
	} control_t;   //maxi:control->thrust 为油门控制量，这个值增大四轴升高，减小则下降。control->roll， control->pitch，control->yaw 为 PID 输出的姿态控制量。
	               //maxi:油门控制量和姿态控制量整合后控制 电机，整合代码在 power_control.c 文件的函数 powerControl ()中实现
	typedef enum   //maxi:这好像是上一个结构体中定义翻滚方向的。
{
	modeDisable = 0,/*关闭模式*/
	modeAbs,		/*绝对值模式*/
	modeVelocity	/*速率模式*/
...
#define RATE_DO_EXECUTE(RATE_HZ, TICK) ((TICK % (MAIN_LOOP_RATE / RATE_HZ)) == 0)  //maxi:就是把MAIN_LOOP_RATE用1000替代掉。
